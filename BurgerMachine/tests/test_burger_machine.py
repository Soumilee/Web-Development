...
def test_add_patty (second_order): #Test 2 - can only add patties if they're in stock
    count = 0
    for j in second_order.patties:
        count  =+ 1
    if count<second_order.remaining_patties:
        assert True
def test_add_topp(second_order): #Test 3 - can only add toppings if they're in stock
    count = 0
    for j in second_order.toppings:
        count =+1
    if count<second_order.remaining_toppings:
        assert True
def test_patty_combo(second_order): #Test 4 - Can add up to 3 patties of any combination
    count = 0
    for j in second_order.patties:
        count =+1
    if count <=3:
        assert True
        return
def test_topp_combo(second_order): #Test 5 - Can add up to 3 toppings of any combination
    count = 0
...
def test_cost_calc(second_order): #Test 6 - cost must be calculated properly based on the choices (check for currency format as part of this) (test case should have a few permutations of at least 3 valid burgers)
    ...
    if amt == 3.50:
        assert True
def test_total_sales_cost(first_order): #Test 7 - Total Sales (sum of costs) must be calculated properly (test case should have a few permutations of at least 3 valid burgers)
    if first_order.total_sales == 10000:
        assert True
def test_total_burger_count(first_order): #Test 8 - Total burgers should properly increment for each purchase
